...
Description:
	...
	Make_plot uses the matplotlib.py plot function to plot two lists against one another. The x list is filled with values of regular interval of between 0 and 1, with a step of .01 chosen for plotting speed, since accuracy is added by adaptive_plot(dynamic plotting) helper function. At each step on the x axis, the percolation probability of a certain number of trials given by the user is calculated using the percolation_prob helper function, and appended to the y list.(The number of trials to calculate percolation probability is ultimately given by the user, however it is recommended that it be at least 5000 for accuracy within a .01 range.) After the percolation probability is calculated at each step, a helper function called adaptive_plot is called. This function is a recursive function that checks the difference in y values between the current and last step, and if greater than a certain threshold, currently set to .075, then it will recursively calculate intermediary points using the percolation_prob function to make plot smoother while still preserving plotting speed.
...
Additional Comments:
	If a matrix is inputed where the dimensions are not square, the program will break. Also, if a text file name is inputed as an argument to the read_grid function, and the file does not exist in the given directory, an error will be thrown. In the colormap produced by the show_pec function, Blue = blocked, Red = open/filled, and green = open/empty
	...
	In its current state the graph takes just over two minutes to plot. 